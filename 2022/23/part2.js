...
const hasProp = elf => !noPropose(elf)
...
require('fs').readFile('./puzzle-input.txt', 'utf-8', (err, data) => {
  ...
  while(true) {
    ...
    //console.log('how many will move', map.filter(hasProp).length)
    if (map.filter(hasProp).length === 0) {
      console.log('The round where noone moves is', round)
      break;
    }
    map.forEach(moveIfAlone)
    map.forEach(reset)
