...
function findNextAllowedValue(generator, divisor) {
  let result
...
function compareGenerators(startA, startB, amount) {
  ...
  function judgeIteration() {
    counter++
    const validA = findNextAllowedValue( generatorA, 4 )
    const validB = findNextAllowedValue( generatorB, 8 )
    judgeTotal += binaryCompare( validA, validB ) ? 1 : 0

    if (counter % 100000 === 0)
      console.log('Simulation at iteration ' + counter)
  }

  times( judgeIteration , 5000000)
